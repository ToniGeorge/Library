...
public class HistoryController implements Initializable {
    ...
    private void fill() {
        ...
        ObservableList<info> hist = FXCollections.observableArrayList();
        ObservableList<info> recom = FXCollections.observableArrayList();
        ...
        for (int i = 0; i < blist.size(); i++) {
            ...
            hist.add(new info(tmp.ISBN, tmp.Title, tmp.Genre));
//            System.out.println(blist.get(i).Author);
        }
        history.setItems(hist);
        for (int l = 0; l < recomm.size(); l++) {
            Book tmp = recomm.get(l);
            recom.add(new info(tmp.ISBN, tmp.Title, tmp.Genre));
            //System.out.println(blist.get(i).Author);
        }
        recommendations.setItems(recom);
